...
// Pointer to the client struct holding the state for your client
// implementation.
// Changed to an opaque pointer
typedef struct ClientImplementation ClientImplementation;
...
// * `addr` - Address or hostname of server. The caller retains ownership of the
// string.
// * `port` - Port number of server.
// * `game_id` - ID of the game to join.
//
// Returns true on successful connection, false otherwise.
extern bool client_connect(ClientImplementation* client, const char* addr,
...
// This function should not return until both player's ship placements have been
// acknowledged by the server.
//
// * `client` - Pointer to client implementation.
// * `ships` - Array of 4 ships to place for the current player. The caller
// retains ownership of the array.
//
...
//
// * `client` - Pointer to client implementation.
// * `coordinate` - Coordinate of the move to perform. The caller retains
...
// Send the player's move to the server, with request for extended move
// encoding.
//
// * `client` - Pointer to client implementation.
// * `coordinate` - Coordinate of the move to perform. The caller retains
...
extern struct ExtendedTurnResult client_send_move_extended(
    ClientImplementation* client, const char* coordinate);

// Receive the opponent's move from the server.
//
// * `client` - Pointer to client implementation.
//
// Returns coordinate and result of the opponent's move, which is freed by the
// caller.
extern struct MoveResult client_receive_move(ClientImplementation* client);

// Frees the data associated with an `ExtendedTurnResult` returned by
// `client_send_move_extended`.
//
// * `result` - The `ExtendedTurnResult` to free.
extern void client_free_extended_result(struct ExtendedTurnResult result);

// Frees the data associated with a `MoveResult` returned by
// `client_receive_move`.
//
